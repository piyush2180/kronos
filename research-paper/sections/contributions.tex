...
\begin{itemize}
    \item \textbf{GC-Neutral Search Architecture:} An implementation that bounds V8 heap usage between 32.85\,MB and 445.66\,MB during tournament play (with single-game telemetry profiling showing active heap usage down to 24.98\,MB) by avoiding hot-path heap allocations via direct in-place board state mutations, flat TypedArray move buffers, and size-capped transposition table eviction.
