...
Our empirical findings establish a clear hierarchy of heuristic impact. Late Move Reduction is the most significant search reduction optimization, contributing a 66.3\% node reduction. Quiescence search is critical, as its deactivation results in a 417 Elo rating regression—the largest performance drop in our ablation study. Move ordering serves as the foundation on which both LMR and PVS depend; without it, pruning heuristics lose their efficiency. Together, the full heuristic stack compresses the search space by 62.0\% at depth 4, while reaching a peak calibrated playing strength of 1,485 Elo at depth 6.

The depth-7 regression to 1,450 Elo highlights a latency ceiling imposed by single-threaded execution. When the search latency exceeds the tournament time limit, the engine falls back to depth-6 evaluations in tactically sharp positions. Overcoming this ceiling requires parallel search, which remains a primary avenue for future work.
...
To reproduce the experimental suite, execute \texttt{npm run benchmark} from the project root. Individual experiments can be executed by specifying a configuration file:
\begin{verbatim}
...
This combination of a reproducible pipeline, statistically validated results, and open-source architecture establishes Kronos as an accessible research platform.
