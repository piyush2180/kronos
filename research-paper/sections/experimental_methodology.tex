...
The SPRT routine evaluates the log-likelihood ratio (LLR) of match outcomes against a null hypothesis $H_0$ ($\text{Elo} \le 0$) and an alternative hypothesis $H_1$ ($\text{Elo} \ge 10$). The runner computes the LLR after each game pair. If the LLR exceeds the upper boundary ($\eta_1 = \log \frac{1-\beta}{\alpha}$), the tournament terminates and accepts $H_1$. Conversely, if the LLR falls below the lower boundary ($\eta_0 = \log \frac{\beta}{1-\alpha}$), the scheduler terminates and accepts $H_0$. Under target error rates of $\alpha = 0.05$ and $\beta = 0.05$, these boundary thresholds correspond to $[-2.944, 2.944]$. Unlike classical null-hypothesis significance testing, this sequential approach does not calculate frequentist $p$-values, but rather provides a binary decision boundary at specified error limits.
