...
As documented in Table~\ref{tab:memory_profile}, early configurations utilizing uncapped transposition tables (EXP-5-TRANSPOSITION and EXP-6-ITERATIVE) suffered from heap inflation, which triggered frequent collection cycles. Restricting transposition table growth to 500,000 entries and wrapping board mutations in the allocation-free \texttt{ChessInstanceClone} class bounded V8 heap memory between 14.8\,MB and 28.4\,MB during standard tournament matches. Within this footprint, V8's minor collections run in under 1\,ms, preventing search loop interruptions.
...
Depth-scaling metrics (Table~\ref{tab:cross_depth_elo}) exhibit consistent strength gains up to depth 6, followed by regression at depth 7. This is a latency-induced limitation of the single-threaded execution model.
